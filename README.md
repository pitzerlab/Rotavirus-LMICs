...
For the suspension scenario, from 2025 onward, the following vaccination-related parameters are set to zero:
...
For the 95% scaled-up coverage scenario, projected vaccine coverage is set to 0.95 (95%) from 2025 onward.
...
| `vaccine_coverage.mat` | Contains two country-specific vaccination-coverage datasets, `vacc_cov_baseline` and `vacc_cov`. |
...
The model is implemented in MATLAB using a main model script and ordinary differential equations.
...
Load the fixed model parameters.

Load the country-specific demographic data.

Load the country-specific vaccination coverage.

Load the country-specific vaccination status.

Apply the selected vaccination scenario.

Run the main model loop across the 112 LMICs.

Use icountry to identify the country within the loop.

Access the corresponding country-specific model inputs.

Generate and save the simulation results.

Continue through the country loop until all 112 countries have been processed.
...
represents the numerical country identifier used within the main model loop.

The country order corresponding to the icountry index is provided in:
...
Download or clone the Rotavirus-LMICs repository.

Open MATLAB.

Set the Rotavirus-LMICs directory as the MATLAB current working directory.

Ensure that all required .m, .mat, and .xlsx files are present in the same directory.

Run the main model script:

main_model_112_LMICs

By default, the script loops across all 112 LMICs.

The country being processed is identified by icountry, and the corresponding country order is provided in icountry_order.xlsx.

The model generates country-specific simulation output files according to the relevant scenario-specific naming conventions.
